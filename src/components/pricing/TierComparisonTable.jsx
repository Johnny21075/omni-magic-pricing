  ...
  return (
    <div>
      ...
      <div className="md:hidden space-y-6">
        {data.tiers.map((tier) => (
          <div key={tier.name} className="bg-slate-800 rounded-lg p-4 border border-slate-700">
            <div className="flex flex-col items-center gap-2 mb-4 pb-4 border-b border-slate-700">
              {tier.badge && (
                <span className="text-xs bg-amber-500 text-slate-900 px-2 py-1 rounded-full font-medium">
                  {tier.badge}
                </span>
              )}
              <h3 className="text-white text-lg font-semibold">{tier.label}</h3>
              <p className="text-slate-400 text-sm text-center">{tier.best_for}</p>
            </div>
            <div className="space-y-3">
              {allFeatures.map((feature) => (
                <div key={feature} className="flex items-center justify-between">
                  <span className="text-slate-300 text-sm">{formatFeatureName(feature)}</span>
                  {tier.features[feature] ? (
